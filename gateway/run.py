...
# Load environment variables from ~/.claudia/.env first.
# User-managed env files should override stale shell exports on restart.
from dotenv import load_dotenv  # backward-compat for tests that monkeypatch this symbol
from claudia_cli.env_loader import load_claudia_dotenv
_env_path = _claudia_home / '.env'
load_claudia_dotenv(claudia_home=_claudia_home, project_env=Path(__file__).resolve().parents[1] / '.env')

# Bridge config.yaml values into the environment so os.getenv() picks them up.
# config.yaml is authoritative for terminal settings — overrides .env.
_config_path = _claudia_home / 'config.yaml'
    ...
    try:
        ...
        if _agent_cfg and isinstance(_agent_cfg, dict):
            if "max_turns" in _agent_cfg:
                os.environ["CLAUDIA_MAX_ITERATIONS"] = str(_agent_cfg["max_turns"])
        # Timezone: bridge config.yaml → CLAUDIA_TIMEZONE env var.
        # CLAUDIA_TIMEZONE from .env takes precedence (already in os.environ).
        _tz_cfg = _cfg.get("timezone", "")
        if _tz_cfg and isinstance(_tz_cfg, str) and "CLAUDIA_TIMEZONE" not in os.environ:
            os.environ["CLAUDIA_TIMEZONE"] = _tz_cfg.strip()
        # Security settings
        _security_cfg = _cfg.get("security", {})
        if isinstance(_security_cfg, dict):
            _redact = _security_cfg.get("redact_secrets")
            if _redact is not None:
                os.environ["CLAUDIA_REDACT_SECRETS"] = str(_redact).lower()
    except Exception:
        pass  # Non-fatal; gateway can still run with .env values

# Gateway runs in quiet mode - suppress debug output and use cwd directly (no temp dirs)
os.environ["CLAUDIA_QUIET"] = "1"
...
def _expand_whatsapp_auth_aliases(identifier: str) -> set:
    ...
    session_dir = _claudia_home / "whatsapp" / "session"
    resolved = set()
    queue = [normalized]
...
def _resolve_runtime_agent_kwargs() -> dict:
    ...
    try:
        runtime = resolve_runtime_provider(
            requested=os.getenv("CLAUDIA_INFERENCE_PROVIDER"),
        )
        ...
        for skill_md in SKILLS_DIR.rglob("SKILL.md"):
            ...
            if name == normalized and name in disabled:
                return (
                    ...
                    f"Enable it with: `claudia skills config`"
                )

        # Check optional skills (shipped with repo but not installed)
        from claudia_constants import get_claudia_home, get_optional_skills_dir
            ...
            for skill_md in optional_dir.rglob("SKILL.md"):
                ...
                if name == normalized:
                    ...
                    return (
                        ...
                        f"Install it with: `claudia skills install {install_path}`"
                    )
    except Exception:
        pass
    return None
...
def _load_gateway_config() -> dict:
    """Load and parse ~/.claudia/config.yaml, returning {} on any error."""
    try:
        config_path = _claudia_home / 'config.yaml'
        if config_path.exists():
            import yaml
            with open(config_path, 'r', encoding='utf-8') as f:
                return yaml.safe_load(f) or {}
    except Exception:
        logger.debug("Could not load gateway config from %s", _claudia_home / 'config.yaml')
    return {}
...
def _resolve_claudia_bin() -> Optional[list[str]]:
    """Resolve the Claudia update command as argv parts.

    Tries in order:
    1. ``shutil.which("claudia")`` — standard PATH lookup
    2. ``sys.executable -m claudia_cli.main`` — fallback when Claudia is running
       from a venv/module invocation and the ``claudia`` shim is not on PATH
    ...
    try:
        ...
        if importlib.util.find_spec("claudia_cli") is not None:
            return [sys.executable, "-m", "claudia_cli.main"]
    except Exception:
        pass
...
class GatewayRunner:
    ...
    def _has_setup_skill(self) -> bool:
        """Check if the claudia-autonomous-setup skill is installed."""
        try:
            from tools.skill_manager_tool import _find_skill
            return _find_skill("claudia-autonomous-setup") is not None
        except Exception:
            return False

    # -- Voice mode persistence ------------------------------------------

    _VOICE_MODE_PATH = _claudia_home / "gateway_voice_mode.json"
    ...
    def _load_prefill_messages() -> List[Dict[str, Any]]:
        ...
        Checks CLAUDIA_PREFILL_MESSAGES_FILE env var first, then falls back to
        the prefill_messages_file key in ~/.claudia/config.yaml.
        Relative paths are resolved from ~/.claudia/.
        """
        import json as _json
        file_path = os.getenv("CLAUDIA_PREFILL_MESSAGES_FILE", "")
        if not file_path:
            try:
                import yaml as _y
                cfg_path = _claudia_home / "config.yaml"
                if cfg_path.exists():
    ...
    def _load_ephemeral_system_prompt() -> str:
        ...
        Checks CLAUDIA_EPHEMERAL_SYSTEM_PROMPT env var first, then falls back to
        agent.system_prompt in ~/.claudia/config.yaml.
        """
        prompt = os.getenv("CLAUDIA_EPHEMERAL_SYSTEM_PROMPT", "")
        if prompt:
            return prompt
        try:
            import yaml as _y
    ...
    def _load_reasoning_config() -> dict | None:
        ...
        CLAUDIA_REASONING_EFFORT as a fallback. Valid: "xhigh", "high",
        ...
        try:
            ...
            pass
        if not effort:
            effort = os.getenv("CLAUDIA_REASONING_EFFORT", "")
    ...
    def _load_background_notifications_mode() -> str:
        ...
        """
        mode = os.getenv("CLAUDIA_BACKGROUND_NOTIFICATIONS", "")
        if not mode:
            try:
                import yaml as _y
                cfg_path = _claudia_home / "config.yaml"
    ...
    async def start(self) -> bool:
        ...
        """
        logger.info("Starting Claudia Gateway...")
        ...
        if not _any_allowlist and not _allow_all:
            logger.warning(
                ...
                "Set GATEWAY_ALLOW_ALL_USERS=true in ~/.claudia/.env to allow open access, "
        ...
        if not notified and any(
            path.exists()
            for path in (
                _claudia_home / ".update_pending.json",
                _claudia_home / ".update_pending.claimed.json",
            )
        ):
    ...
    def _create_adapter(
        ...
        if platform == Platform.TELEGRAM:
            ...
            if not check_slack_requirements():
                logger.warning("Slack: slack-bolt not installed. Run: pip install 'claudia-autonomous[slack]'")
                return None
    ...
    async def _handle_message(self, event: MessageEvent) -> Optional[str]:
                ...
                if code:
                    ...
                    if adapter:
                        await adapter.send(
                            ...
                            f"`claudia pairing approve {platform_name} {code}`"
                        )
                else:
                    adapter = self.adapters.get(source.platform)
                    if adapter:
        ...
        _STALE_TTL = float(os.getenv("CLAUDIA_AGENT_TIMEOUT", 600)) + 60  # timeout + 1 min grace
        ...
        if _quick_key in self._running_agents:
            ...
            from claudia_cli.commands import resolve_command as _resolve_cmd_inner
        ...
        # in claudia_cli/commands.py — no hardcoded set to maintain here.
        from claudia_cli.commands import GATEWAY_KNOWN_COMMANDS, resolve_command as _resolve_cmd
    ...
    async def _handle_message_with_agent(self, event, source, _quick_key: str):
        ...
        if history and len(history) >= 4:
            ...
            _hyg_api_key = None
            try:
                _hyg_cfg_path = _claudia_home / "config.yaml"
            ...
            if not os.getenv(env_key):
                ...
                if adapter:
                    await adapter.send(
                        ...
                        f"A home channel is where Claudia delivers cron job results "
                    ...
                    if _stt_adapter:
                        try:
                            _stt_msg = (
                                ...
                                "(`pip install faster-whisper` in the Claudia venv) "
                                "and set `stt.enabled: true` in config.yaml, "
                                "then /restart the gateway."
                            )
                            # Point to setup skill if it's installed
                            if self._has_setup_skill():
                                _stt_msg += "\n\nFor full setup instructions, type: `/skill claudia-autonomous-setup`"
    ...
    async def _handle_profile_command(self, event: MessageEvent) -> str:
        ...
        # Detect profile name from CLAUDIA_HOME path
        # Profile paths look like: ~/.claudia/profiles/<name>
    ...
    async def _handle_help_command(self, event: MessageEvent) -> str:
        ...
        from claudia_cli.commands import gateway_help_lines
        lines = [
            "📖 **Claudia Commands**\n",
            *gateway_help_lines(),
        ]
    ...
    async def _handle_provider_command(self, event: MessageEvent) -> str:
        ...
        lines.append("Setup: `claudia setup`")
        return "\n".join(lines)
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        ...
        if not personalities:
            return "No personalities configured in `~/.claudia/config.yaml`"
        ...
        try:
            ...
            if "pynacl" in err_lower or "nacl" in err_lower or "davey" in err_lower:
                return (
                    ...
                    "Install or reinstall Claudia with the messaging extra, e.g. "
                    "`pip install claudia-autonomous[messaging]`."
                )
    ...
    async def _handle_rollback_command(self, event: MessageEvent) -> str:
        ...
        try:
            import yaml as _y
            _cfg_path = _claudia_home / "config.yaml"
            if _cfg_path.exists():
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        """Handle /update command — update Claudia to the latest version.

        Spawns ``claudia update`` in a detached session (via ``setsid``) so it
        survives the gateway restart that ``claudia update`` may trigger. Marker
        ...
        import subprocess
        from datetime import datetime
        from claudia_cli.config import is_managed, format_managed_message

        if is_managed():
            return f"✗ {format_managed_message('update Claudia')}"
        ...
        claudia_cmd = _resolve_claudia_bin()
        if not claudia_cmd:
            return (
                "✗ Could not locate the `claudia` command. "
                "Claudia is running, but the update command could not find the "
                "executable on PATH or via the current Python interpreter. "
                "Try running `claudia update` manually in your terminal."
            )

        pending_path = _claudia_home / ".update_pending.json"
        output_path = _claudia_home / ".update_output.txt"
        exit_code_path = _claudia_home / ".update_exit_code"
        pending = {
        ...
        # Spawn `claudia update` detached so it survives gateway restart.
        # Use setsid for portable session detach (works under system services
        # where systemd-run --user fails due to missing D-Bus session).
        claudia_cmd_str = " ".join(shlex.quote(part) for part in claudia_cmd)
        update_cmd = (
            f"{claudia_cmd_str} update > {shlex.quote(str(output_path))} 2>&1; "
        ...
        self._schedule_update_notification_watch()
        return "⚕ Starting Claudia update… I'll notify you when it's done."
    ...
    async def _watch_for_update_completion(
        ...
        """Wait for ``claudia update`` to finish, then send its notification."""
        pending_path = _claudia_home / ".update_pending.json"
        claimed_path = _claudia_home / ".update_pending.claimed.json"
        exit_code_path = _claudia_home / ".update_exit_code"
            ...
            if adapter and chat_id:
                ...
                if output:
                    if len(output) > 3500:
                        output = "…" + output[-3500:]
                    if exit_code == 0:
                        msg = f"✅ Claudia update finished.\n\n```\n{output}\n```"
                    else:
                        msg = f"❌ Claudia update failed.\n\n```\n{output}\n```"
                else:
                    if exit_code == 0:
                        msg = "✅ Claudia update finished successfully."
                    else:
                        msg = "❌ Claudia update failed. Check the gateway logs or run `claudia update` manually for details."
    ...
    def _set_session_env(self, context: SessionContext) -> None:
        ...
        os.environ["CLAUDIA_SESSION_PLATFORM"] = context.source.platform.value
        os.environ["CLAUDIA_SESSION_CHAT_ID"] = context.source.chat_id
        if context.source.chat_name:
            os.environ["CLAUDIA_SESSION_CHAT_NAME"] = context.source.chat_name
        if context.source.thread_id:
            os.environ["CLAUDIA_SESSION_THREAD_ID"] = str(context.source.thread_id)
    
    def _clear_session_env(self) -> None:
        """Clear session environment variables."""
        for var in ["CLAUDIA_SESSION_PLATFORM", "CLAUDIA_SESSION_CHAT_ID", "CLAUDIA_SESSION_CHAT_NAME", "CLAUDIA_SESSION_THREAD_ID"]:
    ...
    async def _enrich_message_with_transcription(
        ...
        if not getattr(self.config, "stt_enabled", True):
            ...
            if self._has_setup_skill():
                disabled_note += (
                    " You have a skill called claudia-autonomous-setup that can help "
                    "users configure Claudia features including voice, tools, and more."
                )
            disabled_note += "]"
            if user_text:
            ...
            try:
                ...
                if result["success"]:
                    ...
                    if (
                        ...
                        if self._has_setup_skill():
                            _no_stt_note += (
                                " You have a skill called claudia-autonomous-setup "
                                "that can help users configure Claudia features "
    ...
    async def _run_agent(
        ...
        progress_mode = (
            _raw_tp
            or os.getenv("CLAUDIA_TOOL_PROGRESS_MODE")
            or "all"
        )
        ...
        def run_sync():
            ...
            os.environ["CLAUDIA_SESSION_KEY"] = session_key or ""
        ...
        try:
            ...
            _agent_timeout = float(os.getenv("CLAUDIA_AGENT_TIMEOUT", 600))
            loop = asyncio.get_event_loop()
            try:
...
def _start_cron_ticker(stop_event: threading.Event, adapters=None, interval: int = 60):
    ...
    needing a separate `claudia cron daemon` or system cron entry.
...
async def start_gateway(config: Optional[GatewayConfig] = None, replace: bool = False, verbosity: Optional[int] = 0) -> bool:
    ...
    # Prevent two gateways from running under the same CLAUDIA_HOME.
    # The PID file is scoped to CLAUDIA_HOME, so future multi-profile
    # setups (each profile using a distinct CLAUDIA_HOME) will naturally
    ...
    if existing_pid is not None and existing_pid != os.getpid():
        if replace:
            ...
            claudia_home = str(get_claudia_home())
            logger.error(
                "Another gateway instance is already running (PID %d, CLAUDIA_HOME=%s). "
                "Use 'claudia gateway restart' to replace it, or 'claudia gateway stop' first.",
                existing_pid, claudia_home,
            )
            print(
                f"\n❌ Gateway already running (PID {existing_pid}).\n"
                f"   Use 'claudia gateway restart' to replace it,\n"
                f"   or 'claudia gateway stop' to kill it first.\n"
                f"   Or use 'claudia gateway run --replace' to auto-replace.\n"
            )
            return False
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Claudia Gateway - Multi-platform messaging")
