...
The PID file lives at ``{CLAUDIA_HOME}/gateway.pid``.  CLAUDIA_HOME defaults to
``~/.claudia`` but can be overridden via the environment variable.  This means
separate CLAUDIA_HOME directories naturally get separate PID files — a property
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting CLAUDIA_HOME."""
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("CLAUDIA_GATEWAY_LOCK_DIR")
    if override:
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different CLAUDIA_HOME dirs).
    """
