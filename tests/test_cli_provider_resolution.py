...
def test_claudia_cli_init_does_not_eagerly_resolve_runtime_provider(monkeypatch):
    cli = _import_cli()
    calls = {"count": 0}

    def _unexpected_runtime_resolve(**kwargs):
        calls["count"] += 1
        raise AssertionError("resolve_runtime_provider should not be called in ClaudiaCLI.__init__")

    monkeypatch.setattr("claudia_cli.runtime_provider.resolve_runtime_provider", _unexpected_runtime_resolve)
    monkeypatch.setattr("claudia_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))

    shell = cli.ClaudiaCLI(model="gpt-5", compact=True, max_turns=1)
...
def test_runtime_resolution_failure_is_not_sticky(monkeypatch):
    ...
    monkeypatch.setattr("claudia_cli.runtime_provider.resolve_runtime_provider", _runtime_resolve)
    monkeypatch.setattr("claudia_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
...
def test_cli_turn_routing_uses_cheap_model_when_simple(monkeypatch):
    ...
    shell = cli.ClaudiaCLI(model="anthropic/claude-sonnet-4", compact=True, max_turns=1)
    shell.provider = "openrouter"
    shell.api_mode = "chat_completions"
    shell.base_url = "https://openrouter.ai/api/v1"
...
def test_cli_prefers_config_provider_over_stale_env_override(monkeypatch):
    ...
    shell = cli.ClaudiaCLI(model="fireworks/minimax-m2p5", compact=True, max_turns=1)
...
def test_codex_provider_replaces_incompatible_default_model(monkeypatch):
    ...
    monkeypatch.setattr("claudia_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
    monkeypatch.setattr(
        "claudia_cli.codex_models.get_codex_model_ids",
        lambda access_token=None: ["gpt-5.2-codex", "gpt-5.1-codex-mini"],
    )

    shell = cli.ClaudiaCLI(compact=True, max_turns=1)
...
def test_model_flow_nous_prints_subscription_guidance_without_mutating_explicit_tts(monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        "claudia_cli.auth.get_provider_auth_state",
    ...
    monkeypatch.setattr(
        "claudia_cli.auth.fetch_nous_models",
        lambda *args, **kwargs: ["claude-opus-4-6"],
    )
    monkeypatch.setattr("claudia_cli.auth._prompt_model_selection", lambda model_ids, current_model="": "claude-opus-4-6")
    monkeypatch.setattr("claudia_cli.auth._save_model_choice", lambda model: None)
    monkeypatch.setattr("claudia_cli.auth._update_config_for_provider", lambda provider, url: None)
    monkeypatch.setattr(
        "claudia_cli.nous_subscription.get_nous_subscription_explainer_lines",
...
def test_codex_provider_preserves_explicit_codex_model(monkeypatch):
    ...
    shell = cli.ClaudiaCLI(model="gpt-5.1-codex-mini", compact=True, max_turns=1)
...
def test_codex_provider_strips_provider_prefix_from_model(monkeypatch):
    ...
    shell = cli.ClaudiaCLI(model="openai/gpt-5.3-codex", compact=True, max_turns=1)
...
def test_cmd_model_falls_back_to_auto_on_invalid_provider(monkeypatch, capsys):
    ...
    monkeypatch.setattr("claudia_cli.config.save_config", lambda cfg: None)
    monkeypatch.setattr("claudia_cli.config.get_env_value", lambda key: "")
...
def test_model_flow_custom_saves_verified_v1_base_url(monkeypatch, capsys):
    monkeypatch.setattr(
        "claudia_cli.config.get_env_value",
        lambda key: "" if key in {"OPENAI_BASE_URL", "OPENAI_API_KEY"} else "",
    )
    saved_env = {}
    monkeypatch.setattr("claudia_cli.config.save_env_value", lambda key, value: saved_env.__setitem__(key, value))
    monkeypatch.setattr("claudia_cli.auth._save_model_choice", lambda model: saved_env.__setitem__("MODEL", model))
    monkeypatch.setattr("claudia_cli.auth.deactivate_provider", lambda: None)
    monkeypatch.setattr("claudia_cli.main._save_custom_provider", lambda *args, **kwargs: None)
    monkeypatch.setattr(
        "claudia_cli.models.probe_api_models",
