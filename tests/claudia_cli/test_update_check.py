"""Tests for the update check mechanism in claudia_cli.banner."""
...
def test_version_string_no_v_prefix():
    ...
    from claudia_cli import __version__
...
def test_check_for_updates_uses_cache(tmp_path):
    ...
    repo_dir = tmp_path / "claudia-autonomous"
    repo_dir.mkdir()
    (repo_dir / ".git").mkdir()
    ...
    with patch("claudia_cli.banner.os.getenv", return_value=str(tmp_path)):
        with patch("claudia_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_expired_cache(tmp_path):
    ...
    with patch("claudia_cli.banner.os.getenv", return_value=str(tmp_path)):
        with patch("claudia_cli.banner.subprocess.run", return_value=mock_result) as mock_run:
            result = check_for_updates()
...
def test_check_for_updates_no_git_dir(tmp_path):
    ...
    fake_banner = tmp_path / "claudia_cli" / "banner.py"
...
def test_check_for_updates_fallback_to_project_root():
    ...
    # Point CLAUDIA_HOME at a temp dir with no claudia-autonomous/.git
    import tempfile
