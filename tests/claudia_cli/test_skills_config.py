"""Tests for claudia_cli/skills_config.py and skills_tool disabled filtering."""
import pytest
...
class TestGetDisabledSkills:
    def test_empty_config(self):
        from claudia_cli.skills_config import get_disabled_skills
...
class TestSaveDisabledSkills:
    @patch("claudia_cli.skills_config.save_config")
    def test_saves_global_sorted(self, mock_save):
        from claudia_cli.skills_config import save_disabled_skills
        config = {}
...
class TestIsSkillDisabled:
    @patch("claudia_cli.config.load_config")
    ...
    @patch("claudia_cli.config.load_config")
    @patch.dict("os.environ", {"CLAUDIA_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_explicit_platform_param(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("CLAUDIA_PLATFORM", raising=False)
        monkeypatch.delenv("CLAUDIA_SESSION_PLATFORM", raising=False)
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """CLAUDIA_SESSION_PLATFORM should be used when CLAUDIA_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
        ...
        monkeypatch.setenv("CLAUDIA_HOME", str(tmp_path))
        monkeypatch.delenv("CLAUDIA_PLATFORM", raising=False)
        monkeypatch.setenv("CLAUDIA_SESSION_PLATFORM", "discord")

        from agent.skill_utils import get_disabled_skill_names
        result = get_disabled_skill_names()
        assert result == {"discord-skill"}

    def test_claudia_platform_takes_precedence(self, tmp_path, monkeypatch):
        """CLAUDIA_PLATFORM should win over CLAUDIA_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
        ...
        monkeypatch.setenv("CLAUDIA_HOME", str(tmp_path))
        monkeypatch.setenv("CLAUDIA_PLATFORM", "telegram")
        monkeypatch.setenv("CLAUDIA_SESSION_PLATFORM", "discord")
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from claudia_cli.skills_config import _get_categories
        skills = [
