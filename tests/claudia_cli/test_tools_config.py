"""Tests for claudia_cli.tools_config platform tool persistence."""

from unittest.mock import patch

from claudia_cli.tools_config import (
    _configure_provider,
    _get_platform_tools,
...
def test_save_platform_tools_preserves_mcp_server_names():
    ...
    Regression test for https://github.com/NousResearch/claudia-autonomous/issues/1247
    """
    config = {
        "platform_toolsets": {
    ...
    with patch("claudia_cli.tools_config.save_config"):
...
def test_save_platform_tools_does_not_preserve_platform_default_toolsets():
    """Platform default toolsets (claudia-cli, claudia-telegram, etc.) must NOT
    ...
    ``claudia tools``, but claudia-cli stays in the config and re-enables
    ...
    config = {
        "platform_toolsets": {
            "cli": [
                ...
                "delegation", "file", "claudia-cli",  # <-- the culprit
    ...
    # claudia-cli must NOT survive — it's a platform default, not an MCP server
    assert "claudia-cli" not in saved
...
def test_save_platform_tools_does_not_preserve_claudia_telegram():
    """Same bug for Telegram — claudia-telegram must not be preserved."""
    config = {
        "platform_toolsets": {
            "telegram": [
                "browser", "file", "claudia-telegram", "terminal", "web",
            ]
        }
    }
    ...
    assert "claudia-telegram" not in saved
    assert "web" in saved
...
def test_save_platform_tools_still_preserves_mcp_with_platform_default_present():
    ...
    config = {
        "platform_toolsets": {
            "cli": [
                "web", "terminal", "claudia-cli", "my-mcp-server", "github-tools",
            ]
        }
    }
...
def test_visible_providers_include_nous_subscription_when_logged_in(monkeypatch):
    ...
    monkeypatch.setattr(
        "claudia_cli.nous_subscription.get_nous_auth_status",
...
def test_local_browser_provider_is_saved_explicitly(monkeypatch):
    ...
    monkeypatch.setattr("claudia_cli.tools_config._run_post_setup", lambda key: None)
...
def test_first_install_nous_auto_configures_managed_defaults(monkeypatch):
    ...
    monkeypatch.setattr(
        "claudia_cli.tools_config._prompt_toolset_checklist",
        lambda *args, **kwargs: {"web", "image_gen", "tts", "browser"},
    )
    monkeypatch.setattr("claudia_cli.tools_config.save_config", lambda config: None)
    monkeypatch.setattr(
        "claudia_cli.nous_subscription.get_nous_auth_status",
        lambda: {"logged_in": True},
    )
    ...
    configured = []
    monkeypatch.setattr(
        "claudia_cli.tools_config._configure_toolset",
...
class TestPlatformToolsetConsistency:
    ...
    def test_all_platforms_have_toolset_definitions(self):
        ...
        from claudia_cli.tools_config import PLATFORMS
    ...
    def test_gateway_toolset_includes_all_messaging_platforms(self):
        """claudia-gateway includes list should cover all messaging platforms."""
        from claudia_cli.tools_config import PLATFORMS
        from toolsets import TOOLSETS

        gateway_includes = set(TOOLSETS["claudia-gateway"]["includes"])
    ...
    def test_skills_config_covers_tools_config_platforms(self):
        ...
        from claudia_cli.tools_config import PLATFORMS as TOOLS_PLATFORMS
        from claudia_cli.skills_config import PLATFORMS as SKILLS_PLATFORMS
