...
import claudia_cli.doctor as doctor
import claudia_cli.gateway as gateway_cli
from claudia_cli import doctor as doctor_mod
from claudia_cli.doctor import _has_provider_env_config
...
def test_run_doctor_sets_interactive_env_for_tool_checks(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(doctor_mod, "CLAUDIA_HOME", claudia_home)
    monkeypatch.delenv("CLAUDIA_INTERACTIVE", raising=False)

    seen = {}

    def fake_check_tool_availability(*args, **kwargs):
        seen["interactive"] = os.getenv("CLAUDIA_INTERACTIVE")
        raise SystemExit(0)
