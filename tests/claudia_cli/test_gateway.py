"""Tests for claudia_cli.gateway."""

import signal
from types import SimpleNamespace
from unittest.mock import patch, call

import claudia_cli.gateway as gateway
...
def test_systemd_install_checks_linger_status(monkeypatch, tmp_path, capsys):
    unit_path = tmp_path / "systemd" / "user" / "claudia-gateway.service"
...
def test_systemd_install_system_scope_skips_linger_and_uses_systemctl(monkeypatch, tmp_path, capsys):
    unit_path = tmp_path / "etc" / "systemd" / "system" / "claudia-gateway.service"
...
def test_conflicting_systemd_units_warning(monkeypatch, tmp_path, capsys):
    user_unit = tmp_path / "user" / "claudia-gateway.service"
    system_unit = tmp_path / "system" / "claudia-gateway.service"
    ...
    assert "claudia gateway uninstall" in out
...
def test_install_linux_gateway_from_setup_system_choice_without_root_prints_followup(monkeypatch, capsys):
    ...
    assert "sudo claudia gateway install --system --run-as-user alice" in out
    assert "sudo claudia gateway start --system" in out
