"""Tests for provider-aware `/model` validation in claudia_cli.models."""
...
def _validate(model, provider="openrouter", api_models=FAKE_API_MODELS, **kw):
    ...
    with patch("claudia_cli.models.fetch_api_models", return_value=api_models), \
         patch("claudia_cli.models.probe_api_models", return_value=probe_payload):
...
class TestProviderModelIds:
    ...
    def test_copilot_prefers_live_catalog(self):
        with patch("claudia_cli.auth.resolve_api_key_provider_credentials", return_value={"api_key": "gh-token"}), \
             patch("claudia_cli.models._fetch_github_models", return_value=["gpt-5.4", "claude-sonnet-4.6"]):
    ...
    def test_copilot_acp_falls_back_to_copilot_defaults(self):
        with patch("claudia_cli.auth.resolve_api_key_provider_credentials", side_effect=Exception("no token")), \
             patch("claudia_cli.models._fetch_github_models", return_value=None):
...
class TestFetchApiModels:
    ...
    def test_returns_none_on_network_error(self):
        with patch("claudia_cli.models.urllib.request.urlopen", side_effect=Exception("timeout")):
    ...
    def test_probe_api_models_tries_v1_fallback(self):
        ...
        with patch("claudia_cli.models.urllib.request.urlopen", side_effect=_fake_urlopen):
    ...
    def test_probe_api_models_uses_copilot_catalog(self):
        ...
        with patch("claudia_cli.models.urllib.request.urlopen", return_value=_Resp()) as mock_urlopen:
    ...
    def test_fetch_github_model_catalog_filters_non_chat_models(self):
        ...
        with patch("claudia_cli.models.urllib.request.urlopen", return_value=_Resp()):
