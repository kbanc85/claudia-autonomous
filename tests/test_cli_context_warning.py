...
def _isolate(tmp_path, monkeypatch):
    """Isolate CLAUDIA_HOME so tests don't touch real config."""
...
def cli_obj(_isolate):
    """Create a minimal ClaudiaCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = ClaudiaCLI.__new__(ClaudiaCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["claudia-core"]
        obj.compact = False
