...
import os
import pytest
from claudia_cli.config import _expand_env_vars, load_config
...
class TestLoadConfigExpansion:
    def test_load_config_expands_env_vars(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("claudia_cli.config.get_config_path", lambda: config_file)
...
class TestLoadCliConfigExpansion:
    ...
    def test_cli_config_expands_auxiliary_api_key(self, tmp_path, monkeypatch):
        ...
        # Patch the claudia home so load_cli_config finds our test config
        monkeypatch.setattr("cli._claudia_home", tmp_path)
