"""Tests for --yolo (CLAUDIA_YOLO_MODE) approval bypass."""
...
class TestYoloMode:
    """When CLAUDIA_YOLO_MODE is set, all dangerous commands are auto-approved."""
    ...
    def test_dangerous_command_blocked_normally(self, monkeypatch):
        ...
        monkeypatch.setenv("CLAUDIA_SESSION_KEY", "test-session")
        monkeypatch.delenv("CLAUDIA_YOLO_MODE", raising=False)
    ...
    def test_dangerous_command_approved_in_yolo_mode(self, monkeypatch):
        """With CLAUDIA_YOLO_MODE, dangerous commands are auto-approved."""
        monkeypatch.setenv("CLAUDIA_YOLO_MODE", "1")
        monkeypatch.setenv("CLAUDIA_INTERACTIVE", "1")
        monkeypatch.setenv("CLAUDIA_SESSION_KEY", "test-session")
    ...
    def test_yolo_mode_not_set_by_default(self):
        """CLAUDIA_YOLO_MODE should not be set by default."""
        # Clean env check — if it happens to be set in test env, that's fine,
        # we just verify the mechanism exists
        assert os.getenv("CLAUDIA_YOLO_MODE") is None or True  # no-op, documents intent

    def test_yolo_mode_empty_string_does_not_bypass(self, monkeypatch):
        """Empty string for CLAUDIA_YOLO_MODE should not trigger bypass."""
        monkeypatch.setenv("CLAUDIA_YOLO_MODE", "")
        monkeypatch.setenv("CLAUDIA_INTERACTIVE", "1")
        monkeypatch.setenv("CLAUDIA_SESSION_KEY", "test-session")

        # Empty string is falsy in Python, so getenv("CLAUDIA_YOLO_MODE") returns ""
