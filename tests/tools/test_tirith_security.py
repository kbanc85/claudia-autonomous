...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._claudia_bin_dir", return_value="/nonexistent"), \
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_claudia_bin(self):
        """After _INSTALL_FAILED, manual install in CLAUDIA_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        claudia_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(claudia_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(claudia_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._claudia_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == claudia_bin
            assert _tirith_mod._resolved_path == claudia_bin
            mock_clear.assert_called_once()
...
class TestClaudiaHomeIsolation:
    def test_claudia_bin_dir_respects_claudia_home(self):
        ...
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"CLAUDIA_HOME": tmpdir}):
            result = _claudia_bin_dir()
    ...
    def test_conftest_isolation_prevents_real_home_writes(self):
        ...
        claudia_home = os.getenv("CLAUDIA_HOME")
        assert claudia_home is not None, "CLAUDIA_HOME should be set by conftest"
        assert "claudia_test" in claudia_home, "Should point to test temp dir"

    def test_get_claudia_home_fallback(self):
        """Without CLAUDIA_HOME set, falls back to ~/.claudia."""
        from tools.tirith_security import _get_claudia_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove CLAUDIA_HOME entirely
            os.environ.pop("CLAUDIA_HOME", None)
            result = _get_claudia_home()
        assert result == os.path.join(os.path.expanduser("~"), ".claudia")
