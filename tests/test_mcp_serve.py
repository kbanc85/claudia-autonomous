"""
Tests for mcp_serve — Claudia MCP server.
...
def _isolate_claudia_home(tmp_path, monkeypatch):
    """Redirect CLAUDIA_HOME to a temp directory."""
    monkeypatch.setenv("CLAUDIA_HOME", str(tmp_path))
    try:
        import claudia_constants
        monkeypatch.setattr(claudia_constants, "get_claudia_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking claudia_state schema."""
