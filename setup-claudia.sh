...
# ============================================================================
# Seed bundled skills into ~/.claudia/skills/
# ============================================================================

CLAUDIA_SKILLS_DIR="${CLAUDIA_HOME:-$HOME/.claudia}/skills"
mkdir -p "$CLAUDIA_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.claudia/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$CLAUDIA_SKILLS_DIR/" 2>/dev/null || true
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m claudia_cli.main setup
fi
