"""Anthropic Messages API adapter for Claudia.

Translates between Claudia's internal OpenAI-style message format and
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Claudia historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Claudia)
    ...
    # 1. Claudia-managed OAuth/setup token env var
    ...
    # This remains as a compatibility fallback for pre-migration Claudia configs.
    api_key = os.getenv("ANTHROPIC_API_KEY", "").strip()
    if api_key:
        return api_key
...
# ── Claudia-native PKCE OAuth flow ────────────────────────────────────────
...
def run_claudia_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Claudia-native OAuth PKCE flow and return credential state."""
    import time
    import webbrowser
    ...
    print()
    print("Authorize Claudia with your Claude Pro/Max subscription.")
    print()
...
def run_claudia_oauth_login() -> Optional[str]:
    """Run Claudia-native OAuth PKCE flow for Claude Pro/Max subscription.
...
def _save_claudia_oauth_credentials(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    ...
    try:
        ...
        logger.debug("Failed to save Claudia OAuth credentials: %s", e)


def read_claudia_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Claudia-managed OAuth credentials from ~/.claudia/.anthropic_oauth.json."""
    if _CLAUDIA_OAUTH_FILE.exists():
        try:
            ...
            logger.debug("Failed to read Claudia OAuth credentials: %s", e)
    return None


def refresh_claudia_oauth_token() -> Optional[str]:
    """Refresh the Claudia-managed OAuth token using the stored refresh token.
    ...
    try:
        ...
        logger.debug("Successfully refreshed Claudia OAuth token")
        return refreshed["access_token"]
    except Exception as e:
        logger.debug("Failed to refresh Claudia OAuth token: %s", e)
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Claudia", "Claude Code")
                text = text.replace("Claudia", "Claude Code")
                text = text.replace("claudia-autonomous", "claude-code")
