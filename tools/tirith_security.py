...
it is automatically downloaded from GitHub releases to $CLAUDIA_HOME/bin/tirith.
...
def _failure_marker_path() -> str:
    ...
    return os.path.join(_get_claudia_home(), ".tirith-install-failed")
...
def _claudia_bin_dir() -> str:
    """Return $CLAUDIA_HOME/bin, creating it if needed."""
    d = os.path.join(_get_claudia_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $CLAUDIA_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_claudia_bin_dir(), "tirith")
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $CLAUDIA_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $CLAUDIA_HOME/bin/tirith
    ...
    claudia_bin = os.path.join(_claudia_bin_dir(), "tirith")
    if os.path.isfile(claudia_bin) and os.access(claudia_bin, os.X_OK):
        _resolved_path = claudia_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return claudia_bin
