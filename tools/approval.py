...
# via shell expansions like $HOME or $CLAUDIA_HOME.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_CLAUDIA_ENV_PATH = (
    r'(?:~\/\.claudia/|'
    r'(?:\$home|\$\{home\})/\.claudia/|'
    r'(?:\$claudia_home|\$\{claudia_home\})/)'
    r'\.env\b'
)
_SENSITIVE_WRITE_TARGET = (
    r'(?:/etc/|/dev/sd|'
    rf'{_SSH_SENSITIVE_PATH}|'
    rf'{_CLAUDIA_ENV_PATH})'
)
...
DANGEROUS_PATTERNS = [
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart claudia-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart claudia-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(claudia|gateway|cli\.py)\b', "kill claudia/gateway process (self-termination)"),
...
def check_dangerous_command(command: str, env_type: str,
    ...
    if os.getenv("CLAUDIA_YOLO_MODE"):
        return {"approved": True, "message": None}
    ...
    session_key = os.getenv("CLAUDIA_SESSION_KEY", "default")
    if is_approved(session_key, pattern_key):
        return {"approved": True, "message": None}

    is_cli = os.getenv("CLAUDIA_INTERACTIVE")
    is_gateway = os.getenv("CLAUDIA_GATEWAY_SESSION")

    if not is_cli and not is_gateway:
        return {"approved": True, "message": None}

    if is_gateway or os.getenv("CLAUDIA_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    if os.getenv("CLAUDIA_YOLO_MODE") or approval_mode == "off":
        return {"approved": True, "message": None}

    is_cli = os.getenv("CLAUDIA_INTERACTIVE")
    is_gateway = os.getenv("CLAUDIA_GATEWAY_SESSION")
    is_ask = os.getenv("CLAUDIA_EXEC_ASK")
