...
Usage in execute_code:
    exec(open(os.path.expanduser(
        "~/.claudia/skills/red-teaming/godmode/scripts/auto_jailbreak.py"
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("CLAUDIA_HOME", Path.home() / ".claudia")) / "skills" / "red-teaming" / "godmode"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Claudia models — already uncensored, just needs clean prompt
    "claudia": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "claudia" in model_lower or "nous" in model_lower:
        return "claudia"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Claudia config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.claudia/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Claudia for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Claudia for changes to take effect.")
