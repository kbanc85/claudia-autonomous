...
- Give it a name like "claudia-autonomous"
- Select scopes:
...
- Give it a title like "claudia-autonomous-<machine-name>"
...
elif [ -f ~/.claudia/.env ] && grep -q "^GITHUB_TOKEN=" ~/.claudia/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.claudia/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
