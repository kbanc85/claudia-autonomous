...
if tinker_atropos_dir.exists():
    ...
    os.environ['CLAUDIA_QUIET'] = '1'  # Disable temp subdirectory creation
    print(f"📂 Terminal working directory: {tinker_atropos_dir}")
else:
    # Fall back to claudia-autonomous directory if submodule not found
...
from claudia_constants import get_claudia_home, OPENROUTER_BASE_URL

DEFAULT_MODEL = "anthropic/claude-opus-4.5"
DEFAULT_BASE_URL = OPENROUTER_BASE_URL


def load_claudia_config() -> dict:
    """
...
def main(
    ...
    Args:
        ...
        model: Model to use for the agent (reads from ~/.claudia/config.yaml if not provided)
    ...
    """
    # Load config from ~/.claudia/config.yaml
    config = load_claudia_config()
