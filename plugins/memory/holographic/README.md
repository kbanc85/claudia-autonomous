...
```bash
claudia memory setup    # select "holographic"
```

Or manually:
```bash
claudia config set memory.provider holographic
```

## Config

Config in `config.yaml` under `plugins.claudia-memory-store`:

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$CLAUDIA_HOME/memory_store.db` | SQLite database path |
