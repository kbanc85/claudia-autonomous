...
```bash
claudia memory setup    # select "byterover"
```

Or manually:
```bash
claudia config set memory.provider byterover
# Optional cloud sync:
echo "BRV_API_KEY=your-key" >> ~/.claudia/.env
```
...
Working directory: `$CLAUDIA_HOME/byterover/` (profile-scoped).
