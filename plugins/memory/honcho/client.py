...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $CLAUDIA_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.claudia/honcho.json          (default profile — shared host blocks live here)
      3. ~/.honcho/config.json          (global, cross-app interop)

    Returns the global path if none exist (for first-time setup writes).
    """
    local_path = get_claudia_home() / "honcho.json"
    if local_path.exists():
        return local_path

    # Default profile's config — host blocks accumulate here via setup/clone
    default_path = Path.home() / ".claudia" / "honcho.json"
...
class HonchoClientConfig:
    ...
    # True when Honcho was explicitly configured for this host (hosts.claudia
    ...
    def from_global_config(
        ...
        # A hosts.claudia block or explicit enabled flag means the user
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not resolved_base_url:
        try:
            ...
            claudia_cfg = load_config()
            honcho_cfg = claudia_cfg.get("honcho", {})
