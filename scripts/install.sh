...
# Configuration
REPO_URL_SSH="git@github.com:kbanc85/claudia-autonomous.git"
REPO_URL_HTTPS="https://github.com/kbanc85/claudia-autonomous.git"
...
print_banner() {
    ...
    echo "│             ⚕ Claudia Installer                    │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  An open source AI agent by Kamil Banc.              │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  irm https://raw.githubusercontent.com/kbanc85/claudia-autonomous/main/scripts/install.ps1 | iex"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_node() {
    ...
    log_info "Extracting to ~/.claudia/node/..."
    ...
    # Place into ~/.claudia/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$CLAUDIA_HOME/node"
    mkdir -p "$CLAUDIA_HOME"
    mv "$extracted_dir" "$CLAUDIA_HOME/node"
    rm -rf "$tmp_dir"

    mkdir -p "$HOME/.local/bin"
    ln -sf "$CLAUDIA_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$CLAUDIA_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$CLAUDIA_HOME/node/bin/npx"  "$HOME/.local/bin/npx"

    export PATH="$CLAUDIA_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$CLAUDIA_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.claudia/node/"
    HAS_NODE=true
}
...
copy_config_templates() {
    ...
    # Create ~/.claudia directory structure (config at top level, code in subdir)
    mkdir -p "$CLAUDIA_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills,whatsapp/session}

    # Create .env at ~/.claudia/.env (top level, easy to find)
    if [ ! -f "$CLAUDIA_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$CLAUDIA_HOME/.env"
            log_success "Created ~/.claudia/.env from template"
        else
            touch "$CLAUDIA_HOME/.env"
            log_success "Created ~/.claudia/.env"
        fi
    else
        log_info "~/.claudia/.env already exists, keeping it"
    fi

    # Create config.yaml at ~/.claudia/config.yaml (top level, easy to find)
    if [ ! -f "$CLAUDIA_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$CLAUDIA_HOME/config.yaml"
            log_success "Created ~/.claudia/config.yaml from template"
        fi
    else
        log_info "~/.claudia/config.yaml already exists, keeping it"
    fi
...
-->
SOUL_EOF
        log_success "Created ~/.claudia/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.claudia/"

    # Seed bundled skills into ~/.claudia/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.claudia/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.claudia/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$CLAUDIA_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$CLAUDIA_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.claudia/skills/"
        fi
    fi
}
...
run_setup_wizard() {
    ...
    if [ "$USE_VENV" = true ]; then
        ...
        python -m claudia_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$CLAUDIA_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    WHATSAPP_SESSION="$CLAUDIA_HOME/whatsapp/session/creds.json"
    ...
    if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
        ...
        if command -v systemctl &> /dev/null; then
            ...
            if $CLAUDIA_CMD gateway install 2>/dev/null; then
            ...
            nohup $CLAUDIA_CMD gateway > "$CLAUDIA_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
...
print_success() {
    ...
    # Show file locations
    echo -e "${CYAN}${BOLD}📁 Your files (all in ~/.claudia/):${NC}"
    echo ""
