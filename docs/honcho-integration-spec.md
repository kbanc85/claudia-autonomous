...
Comparison of Claudia vs. openclaw-honcho — and a porting spec for bringing Claudia patterns into other Honcho integrations.

---

## Overview

Two independent Honcho integrations have been built for two different agent runtimes: **Claudia** (Python, baked into the runner) and **openclaw-honcho** (TypeScript plugin via hook/tool API). Both use the same Honcho peer paradigm — dual peer model, `session.context()`, `peer.chat()` — but they made different tradeoffs at every layer.

This document maps those tradeoffs and defines a porting spec: a set of Claudia-originated patterns, each stated as an integration-agnostic interface, that any Honcho integration can adopt regardless of runtime or language.

> **Scope** Both integrations work correctly today. This spec is about the delta — patterns in Claudia that are worth propagating and patterns in openclaw-honcho that Claudia should eventually adopt. The spec is additive, not prescriptive.

---

## Architecture comparison

### Claudia: baked-in runner
...
| Dimension | Claudia | openclaw-honcho |
|---|---|---|
...
| **AI peer name in identity** | Replaces "Claudia" in DEFAULT_AGENT_IDENTITY when configured. | Not implemented. |
...
Six patterns from Claudia are worth adopting in any Honcho integration. Each is described as an integration-agnostic interface.
...
**openclaw-honcho contributes back (Claudia should adopt):**
...
- [ ] **honcho mode / sessions / map** — CLI parity with Claudia
...
Greenfield integration. Start from openclaw-honcho's architecture and apply all Claudia patterns from day one.
