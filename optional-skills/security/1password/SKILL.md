...
version: 1.0.0
author: arceus77-7, enhanced by Claudia
license: MIT
metadata:
  claudia:
...
- `tmux` available for stable authenticated sessions during Claudia terminal calls (desktop app flow only)
...
### Service Account (recommended for Claudia)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.claudia/.env` (the skill will prompt for this on first load).
...
## Claudia Execution Pattern (desktop app flow)

Claudia terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/claudia-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/claudia-op.sock"
