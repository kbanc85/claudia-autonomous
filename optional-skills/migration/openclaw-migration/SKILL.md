...
description: Migrate a user's OpenClaw customization footprint into Claudia. Imports Claudia-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.
version: 1.0.0
author: Claudia (Nous Research)
license: MIT
metadata:
  claudia:
    tags: [Migration, OpenClaw, Claudia, Memory, Persona, Import]
    related_skills: [claudia-autonomous]
---

# OpenClaw -> Claudia Migration

Use this skill when a user wants to move their OpenClaw setup into Claudia with minimal manual cleanup.
...
```bash
claudia migrate migrate              # Full interactive migration
...
claudia migrate migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `claudia setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_claudia.py` to:

- import `SOUL.md` into the Claudia home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Claudia memory entries
- merge OpenClaw command approval patterns into Claudia `command_allowlist`
- migrate Claudia-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS` and `MESSAGING_CWD`
- copy OpenClaw skills into `~/.claudia/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Claudia workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.claudia/tts/`
- archive non-secret docs that do not have a direct Claudia destination
...
- `scripts/openclaw_to_claudia.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.claudia/skills/migration/openclaw-migration/scripts/openclaw_to_claudia.py`

Do not guess a shorter path like `~/.claudia/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.claudia/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Claudia-compatible secrets, currently:
...
Claudia CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Claudia. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Claudia workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Claudia skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.claudia/skills/migration/openclaw-migration/scripts/openclaw_to_claudia.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.claudia/skills/migration/openclaw-migration/scripts/openclaw_to_claudia.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.claudia/skills/migration/openclaw-migration/scripts/openclaw_to_claudia.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.claudia/skills/migration/openclaw-migration/scripts/openclaw_to_claudia.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.claudia/skills/migration/openclaw-migration/scripts/openclaw_to_claudia.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.claudia/skills/migration/openclaw-migration/scripts/openclaw_to_claudia.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Claudia unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Claudia targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Claudia destination. Unsupported auth blobs must still be reported as skipped.
...
- Claudia persona state imported
- Claudia memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.claudia/skills/openclaw-imports/`
