...
- smoke-test a server with the FastMCP CLI before wiring it into Claudia or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Claudia. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.claudia/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.claudia/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Claudia integration, either:

- configure the server in `~/.claudia/config.yaml` using the `native-mcp` skill, or
...
### Claudia cannot see the deployed server

The server-building part may be correct while the Claudia config is not. Load the `native-mcp` skill and configure the server in `~/.claudia/config.yaml`, then restart Claudia.
