"""
Claudia MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Claudia-specific extra)

Usage:
    claudia mcp serve
    claudia mcp serve --verbose
...
logger = logging.getLogger("claudia.mcp_serve")
...
class EventBridge:
    ...
    This is the Claudia equivalent of OpenClaw's WebSocket gateway bridge.
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Claudia MCP server with all tools registered."""
    if not _MCP_SERVER_AVAILABLE:
        raise ImportError(
            "MCP server requires the 'mcp' package. "
            "Install with: pip install 'claudia-autonomous[mcp]'"
        )

    mcp = FastMCP(
        "claudia",
        instructions=(
            "Claudia messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Claudia MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
