...
from claudia_cli import __version__, __release_date__
...
def cmd_chat(args):
    ...
    try:
        from claudia_cli.banner import prefetch_update_check
...
def cmd_gateway(args):
    ...
    from claudia_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    import subprocess
    from pathlib import Path
    from claudia_cli.config import get_env_value, save_env_value
...
def cmd_setup(args):
    ...
    _require_tty("setup")
    from claudia_cli.setup import run_setup_wizard
...
def select_provider_and_model(args=None):
    ...
    from claudia_cli.config import load_config, get_env_value
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from claudia_cli.auth import _prompt_model_selection, _save_model_choice, deactivate_provider
    from claudia_cli.config import get_env_value, save_env_value
    ...
    from claudia_cli.models import model_ids
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from claudia_cli.config import get_env_value, save_config, save_env_value
    from claudia_cli.nous_subscription import (
...
def _model_flow_custom(config):
    ...
    """
    from claudia_cli.auth import _save_model_choice, deactivate_provider
    from claudia_cli.config import get_env_value, save_env_value, load_config, save_config
    ...
    from claudia_cli.models import probe_api_models
...
def _model_flow_named_custom(config, provider_info):
    ...
    """
    from claudia_cli.auth import _save_model_choice, deactivate_provider
    from claudia_cli.config import save_env_value, load_config, save_config
    from claudia_cli.models import fetch_api_models
...
def _model_flow_copilot(config, current_model=""):
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from claudia_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from claudia_cli.copilot_auth import validate_copilot_token
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    from claudia_cli.config import get_env_value, save_env_value, load_config, save_config
    from claudia_cli.models import fetch_api_models, opencode_model_api_mode, normalize_opencode_model_id
...
def cmd_logout(args):
    ...
    from claudia_cli.auth import logout_command
    logout_command(args)


def cmd_auth(args):
    """Manage pooled credentials."""
    from claudia_cli.auth_commands import auth_command
    auth_command(args)


def cmd_status(args):
    """Show status of all components."""
    from claudia_cli.status import show_status
    show_status(args)


def cmd_cron(args):
    """Cron job management."""
    from claudia_cli.cron import cron_command
    cron_command(args)


def cmd_webhook(args):
    """Webhook subscription management."""
    from claudia_cli.webhook import webhook_command
    webhook_command(args)


def cmd_doctor(args):
    """Check configuration and dependencies."""
    from claudia_cli.doctor import run_doctor
    run_doctor(args)


def cmd_config(args):
    """Configuration management."""
    from claudia_cli.config import config_command
    config_command(args)
...
def cmd_version(args):
    ...
    try:
        from claudia_cli.banner import check_for_updates
        from claudia_cli.config import recommended_update_command
...
def cmd_update(args):
    ...
    try:
        ...
        try:
            from claudia_cli.profiles import list_profiles, get_active_profile_name, seed_profile_skills
        ...
        try:
            ...
            from claudia_cli.gateway import (
            ...
            if is_macos():
                try:
                    from claudia_cli.gateway import get_launchd_label
...
def cmd_profile(args):
    ...
    from claudia_cli.profiles import (
    ...
    if action == "list":
        ...
        name = args.profile_name
        from claudia_cli.profiles import get_profile_dir, profile_exists, _read_config_model, _check_gateway_running, _count_skills
        ...
        from claudia_cli.profiles import profile_exists
        if not profile_exists(name):
            print(f"Error: Profile '{name}' does not exist.")
            sys.exit(1)
        ...
        from claudia_cli.profiles import rename_profile
        try:
        ...
        from claudia_cli.profiles import export_profile
        name = args.profile_name
        ...
        from claudia_cli.profiles import import_profile
        try:
...
def cmd_completion(args):
    ...
    from claudia_cli.profiles import generate_bash_completion, generate_zsh_completion
...
"""
    ...
    def cmd_pairing(args):
        from claudia_cli.pairing import pairing_command
    ...
    def cmd_skills(args):
        ...
        if getattr(args, 'skills_action', None) == 'config':
            ...
            from claudia_cli.skills_config import skills_command as skills_config_command
            skills_config_command(args)
        else:
            from claudia_cli.skills_hub import skills_command
            skills_command(args)
    ...
    def cmd_plugins(args):
        from claudia_cli.plugins_cmd import plugins_command
    ...
    def cmd_tools(args):
        ...
        if action in ("list", "disable", "enable"):
            ...
            _require_tty("tools")
            from claudia_cli.tools_config import tools_command
            tools_command(args)
    ...
    def cmd_mcp(args):
        from claudia_cli.mcp_config import mcp_command
        mcp_command(args)
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            if claudia_bin:
                ...
                os.execvp(
                    sys.executable,
                    [sys.executable, "-m", "claudia_cli.main", "--resume", selected_id],
                )
    ...
    def cmd_claw(args):
        from claudia_cli.claw import claw_command
        claw_command(args)
